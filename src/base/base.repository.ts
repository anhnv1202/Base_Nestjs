...
export class BaseRepository<T extends Document> {
  ...
  async findOne(
    ...
    return await this._addPopulate(this.model.findOne(query, projection), populate);
  }
  ...
  async findOneAndUpdate(
    ...
    return await this._addPopulate(this.model.findOneAndUpdate(query, update, options), populate);
  }
  ...
  async paginate(params: FindAndCountQuery): Promise<[T[], number]> {
    ...
    const { size, page, dateFrom, dateTo, sortBy = 'createdAt', sortType = 'desc', text, ...rest } = pagination;
    const conditions: any = { ...rest };

    if (dateFrom || dateTo) {
      conditions['createdAt'] = {};
      if (dateFrom) conditions['createdAt']['$gte'] = new Date(dateFrom);
      if (dateTo) conditions['createdAt']['$lte'] = new Date(dateTo);
    }
    ...
    let query = this._addPopulate(
      this.model
        .find(conditions)
        .select(select || '-password -twofa.secret -__v')
        .sort({ [sortBy]: sortType.toLocaleLowerCase() === 'desc' ? -1 : 1, _id: 1 }),
      populates,
    );
    ...
    query = query.skip((Number(page) - 1) * Number(size)).limit(Number(size));
    const [data, total] = await Promise.all([query, this.model.countDocuments(conditions)]);
    return [data, total];
  }

  async countDocuments(conditions: FilterQuery<T>): Promise<number> {
